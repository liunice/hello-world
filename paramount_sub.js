(async () => {
    ...
    if (/link\.theplatform\.com\/s\/\w+\/\w+\?format=SMIL/.test($request.url)) {
        ...
        // iOS HDR => #EXT-X-STREAM-INF:BANDWIDTH=12791948,AVERAGE-BANDWIDTH=9801831,CODECS="dvh1.05.06,ec-3",RESOLUTION=3840x2160,FRAME-RATE=23.976,VIDEO-RANGE=PQ,AUDIO="audio_ec3",SUBTITLES="cbsi_webvtt",CLOSED-CAPTIONS=NONE
        // tvOS SDR => #EXT-X-STREAM-INF:BANDWIDTH=6107321,RESOLUTION=1920x1080,CODECS="avc1.640028,ec-3",AUDIO="audio_ec3",SUBTITLES="subs"
        ...
        if (body.indexOf('.ts') > -1) {
            ...
            if (hd_url && $request.url != hd_url) {
                ...
                body = body.replace(/(\s+)(seg_\d+\.ts)/g, '$1' + hd_url.replace('stream.m3u8', '') + '$2')
            }
        }
