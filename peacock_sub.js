(async () => {
    ...
    if (/\/adapter\-calypso\/v\d+\/query\/node.*?represent=\(items/.test($request.url)) {
        ...
        const seasonNo = root['season'].toString();
        const episodenNo = root['episode'].toString();
        $.setdata(seriesName, 'peacock_seriesName');
        ...
        const seasonNo = $.getdata("peacock_seasonNo").padStart(2, '0')
        const epNo = $.getdata("peacock_epNo").padStart(2, '0')
...
#EXTINF:36000.000
https://g001-vod-us-cmaf-prd-cl.cdn.peacocktv.com/pub/global/DzG/2Sw/PCK_1623143256129.5266_01/cmaf/mpeg_cbcs/_777885650.subtitles.0.split.0.webvtt
#EXT-X-ENDLIST
            `
        }
            ...
            episodes.push(
                {
                    ...
                    //"asset_id": ep['attributes']['providerVariantId'],//merlinAlternateId
                    "asset_id": ep['id'],
                    "series_id": ep['attributes']['seriesId'],
                    "series_name": ep['attributes']['seriesName'],
                    "episode_name": ep['attributes']['episodeName']
                }
            )
        }
        const opts = {
            'url': 'https://api.miffysoft.cn/tv_shows/episodes/add?platform=peacock',
